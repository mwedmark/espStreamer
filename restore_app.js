...
// 1. Restore missing tail
const orig = fs.readFileSync('temp_app.js', 'utf8');
...
// 2. Remove console.logs in the restored part
curr = curr.replace(/console\.log\([^)]*\);?/g, '');
curr = curr.replace(/console\.error\([^)]*\);?/g, '');

// 3. Fix the double if(r && r.ok) in upd()
...
if (startIndex !== -1 && catchIndex !== -1) {
    ...
    console.log('Could not find stream logic bounds.');
}
