...
async function save(t) {
  ...
  if (isIFLI) {
    ...
    const asm = [0x78,0xD8,0xA9,0x37,0x85,0x01,0xA2,0x00,0x8A,0x29,0x07,0x09,0x38,0x9D,0x00,0x09,0x8A,0x29,0x07,0x0A,0x0A,0x0A,0x0A,0x09,0x08,0x9D,0x00,0x0A,0xE8,0xE0,0xC8,0xD0,0xE7,0xA2,0x00,0xBD,0x00,0x10,0x9D,0x00,0xD8,0xBD,0xFA,0x10,0x9D,0xFA,0xD8,0xBD,0xF4,0x11,0x9D,0xF4,0xD9,0xBD,0xEE,0x12,0x9D,0xEE,0xDA,0xE8,0xE0,0xFA,0xD0,0xE3,0xA9,0x3B,0x8D,0x11,0xD0,0xA9,0xD8,0x8D,0x16,0xD0,0xA9,currentBgColor,0x8D,0x20,0xD0,0x8D,0x21,0xD0,0xA9,0x08,0x8D,0x18,0xD0,0xA9,0x02,0x8D,0x00,0xDD,0xAD,0x12,0xD0,0xC9,0xF8,0xD0,0xF9,0xAD,0x11,0xD0,0x30,0xFB,0xAD,0x12,0xD0,0xC9,0x2F,0xD0,0xF9,0xA2,0x08,0xCA,0xD0,0xFD,0xEA,0xA0,0x00,0xB9,0x00,0x09,0x8D,0x11,0xD0,0xB9,0x00,0x0A,0x8D,0x18,0xD0,0xC8,0xC0,0xC8,0xD0,0xEF,0x4C,0x67,0x08];
    ...
    const asm = [0x78,0xD8,0xA9,0x37,0x85,0x01,0xA9,0x3B,0x8D,0x11,0xD0,0xA9,(isHires?0xC8:0xD8),0x8D,0x16,0xD0,0xA9,0x18,0x8D,0x18,0xD0,0xA9,currentBgColor,0x8D,0x20,0xD0,0x8D,0x21,0xD0,0xA2,0x00,0xBD,0x76,0x08,0x9D,0x00,0x04,0xBD,0x70,0x09,0x9D,0xFA,0x04,0xBD,0x6A,0x0A,0x9D,0xF4,0x05,0xBD,0x64,0x0B,0x9D,0xEE,0x06,0xE8,0xE0,0xFA,0xD0,0xE3,0xA2,0x00,0xBD,0x5E,0x0C,0x9D,0x00,0xD8,0xBD,0x58,0x0D,0x9D,0xFA,0xD8,0xBD,0x52,0x0E,0x9D,0xF4,0xD9,0xBD,0x4C,0x0F,0x9D,0xEE,0xDA,0xE8,0xE0,0xFA,0xD0,0xE3,0x4C,0x64,0x08];
  ...
  if (t === 'PRG') { download(f, isIFLI ? 'ifli.prg' : (isFLI ? 'fli.prg' : (isHires ? 'hires.prg' : 'v.prg'))); }
    ...
    const phase2 = [
      0xA9,0x01,0x85,0xFD,  // $0200 LDA #1; STA $FD (bank counter)
      0xA9,0x01,0x85,0xFB,  // $0204 LDA #1; STA $FB (dst lo $0801)
      0xA9,0x08,0x85,0xFC,  // $0208 LDA #8; STA $FC (dst hi)
      // copy_bank ($020C):
      ...
      // done: disable cart, JMP $0801
      0xA9,0x03,0x8D,0x02,0xDE,0x8D,0xFF,0xDF, // LDA #3; STA $DE02; STA $DFFF
      0x4C,0x01,0x08            // JMP $0801
    ];

    // Update Phase 1 with actual Phase 2 length and start offset
    phase1[10] = phase2.length - 1;
    phase1[12] = 13 + 10; // offset $17 (23 decimal)
