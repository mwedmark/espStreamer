...
async function save(t) {
  ...
  if (t === 'KOA') {
    ...
    if (isHires) {
      // Hi-res PRG machine code
      // $D011 = $1B (bitmap on, screen on)
      // $D016 = $C8 (standard, no multicolor)
      // $D018 = $18 (screen $0400, bitmap $2000)
      const prgAsm = [
        0x78,                               // SEI
        0xA9, 0x1B, 0x8D, 0x11, 0xD0,      // LDA #$1B, STA $D011 (hi-res bitmap)
        0xA9, 0xC8, 0x8D, 0x16, 0xD0,      // LDA #$C8, STA $D016 (no multicolor)
        0xA9, 0x18, 0x8D, 0x18, 0xD0,      // LDA #$18, STA $D018 (bitmap @$2000)
        0xA9, 0x00, 0x8D, 0x20, 0xD0, 0x8D, 0x21, 0xD0, // border/bg = black
        0xA2, 0x00,                         // LDX #0
        // Copy screen RAM (foreground=white $1, bg=black $0 -> $10 per cell)
        0xA9, 0x10, 0x9D, 0x00, 0x04,      // LDA #$10, STA $0400,X
        0xE8, 0xE0, 0xE8, 0xD0, 0xF8,      // INX, CPX #232, BNE (-8)
        0xA2, 0x00,
        0xA9, 0x10, 0x9D, 0xE8, 0x04,      // next 25 cells (1000-768=232, rem=24 wrap)
        0xE8, 0xE0, 0x18, 0xD0, 0xF8,      // INX, CPX #24, BNE
        // Copy color RAM: all $01 (white)
        0xA2, 0x00,
        0xA9, 0x01, 0x9D, 0x00, 0xD8,      // STA $D800,X
        0xE8, 0xD0, 0xFB,                   // INX, BNE (256 bytes)
        0xA2, 0x00,
        0xA9, 0x01, 0x9D, 0x00, 0xD9,
        0xE8, 0xD0, 0xFB,
        0xA2, 0x00,
        0xA9, 0x01, 0x9D, 0x00, 0xDA,
        0xE8, 0xD0, 0xFB,
        0xA2, 0x00,
        0xA9, 0x01, 0x9D, 0x00, 0xDB,
        0xE8, 0xE0, 0xE8, 0xD0, 0xFB,      // only 1000 cells = 3*256+232
        0x4C, 0x63, 0x08                    // JMP $0863 (infinite loop)
      ];
      f.set(prgAsm, 14);
    } else {
      // Multi-color PRG machine code (original)
      const prgAsm = [
        0x78,
        0xA9, 0x3B, 0x8D, 0x11, 0xD0,
        0xA9, 0xD8, 0x8D, 0x16, 0xD0,
        0xA9, 0x18, 0x8D, 0x18, 0xD0,
